...
export class ClaudeCodeManager {
    ...
    /**
     * 通用加载 Provider 配置到 Claude Code
     * Claude Code 仅支持 Anthropic 协议，因此只支持 anthropic 和 glm_coding_plan* 两种 provider
     * @param providerId Provider ID
     * @param providerDef Provider 定义对象
     * @param userConfig 用户配置，包含 apiKey 等
     */
    loadProviderConfig(providerId, providerDef, userConfig) {
        if (providerId === 'anthropic') {
            ...
            // 2. 清理 shell rc 文件中的 ANTHROPIC_API_KEY 和 ANTHROPIC_BASE_URL
            this.cleanupShellEnvVars();
            // 3. 加载 Anthropic 原生配置到 settings.json
            const currentSettings = this.getSettings();
            // 移除 GLM 相关变量（如果存在），统一使用 ANTHROPIC_API_KEY
            const currentEnv = currentSettings.env || {};
            const { ANTHROPIC_AUTH_TOKEN: _1, ANTHROPIC_BASE_URL: _2, ...cleanedEnv } = currentEnv;
            const anthropicConfig = {
                ...currentSettings,
                env: {
                    ...cleanedEnv,
                    ANTHROPIC_API_KEY: userConfig.apiKey,
                    // 不设置 ANTHROPIC_BASE_URL，使用默认的 Anthropic 官方 API
                    API_TIMEOUT_MS: '3000000',
                    CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC: 1
                }
            };
            this.saveSettings(anthropicConfig);
        }
        else if (providerId.startsWith('glm_coding_plan')) {
            // 使用现有的 GLM 配置方法
            this.loadGLMConfig(providerId, userConfig.apiKey);
        }
        else {
            // 其他 provider（openai, deepseek, local 等）不支持
            const name = providerDef?.name || providerId;
            console.log(`\x1b[33mClaude Code 仅支持 Anthropic 协议。Provider '${name}' 使用不同协议，无法直接配置。建议使用 OpenCode 工具。\x1b[0m`);
        }
    }
    /**
     * 通用卸载 Provider 配置从 Claude Code
     * @param providerId Provider ID
     */
    unloadProviderConfig(providerId) {
        if (providerId === 'anthropic') {
            // 清除 Anthropic 原生配置的环境变量
            const currentSettings = this.getSettings();
            if (!currentSettings.env) {
                return;
            }
            const { ANTHROPIC_API_KEY: _1, ANTHROPIC_BASE_URL: _2, ...otherEnv } = currentSettings.env;
            const newSettings = {
                ...currentSettings,
                env: otherEnv
            };
            ...
            console.log(`\x1b[33mProvider '${providerId}' 无需卸载，Claude Code 未配置该 Provider。\x1b[0m`);
        }
    }
    /**
     * 检查 MCP 服务是否已安装
    ...
    detectCurrentConfig() {
        try {
            ...
            // Check for Anthropic native (ANTHROPIC_API_KEY without ANTHROPIC_BASE_URL)
            if (settings.env.ANTHROPIC_API_KEY && !settings.env.ANTHROPIC_BASE_URL) {
                return {
            ...
            // Check for Anthropic API key with custom BASE_URL (e.g., OpenRouter)
            if (settings.env.ANTHROPIC_API_KEY && settings.env.ANTHROPIC_BASE_URL) {
                const baseUrl = settings.env.ANTHROPIC_BASE_URL;
                if (baseUrl === 'https://api.z.ai/api/anthropic') {
                    return { providerId: 'glm_coding_plan_global', apiKey: settings.env.ANTHROPIC_API_KEY, modelName: null };
                }
                if (baseUrl === 'https://open.bigmodel.cn/api/anthropic') {
                    return { providerId: 'glm_coding_plan_china', apiKey: settings.env.ANTHROPIC_API_KEY, modelName: null };
                }
                return { providerId: 'anthropic', apiKey: settings.env.ANTHROPIC_API_KEY, modelName: null };
            }
            // Check for GLM via ANTHROPIC_AUTH_TOKEN + BASE_URL
            if (settings.env.ANTHROPIC_AUTH_TOKEN && settings.env.ANTHROPIC_BASE_URL) {
                const baseUrl = settings.env.ANTHROPIC_BASE_URL;
                if (baseUrl === 'https://api.z.ai/api/anthropic') {
                    return { providerId: 'glm_coding_plan_global', apiKey: settings.env.ANTHROPIC_AUTH_TOKEN, modelName: null };
                }
                if (baseUrl === 'https://open.bigmodel.cn/api/anthropic') {
                    return { providerId: 'glm_coding_plan_china', apiKey: settings.env.ANTHROPIC_AUTH_TOKEN, modelName: null };
                }
                return { providerId: 'anthropic', apiKey: settings.env.ANTHROPIC_AUTH_TOKEN, modelName: null };
            }
            return { providerId: null, apiKey: null, modelName: null };
        }
        catch {
