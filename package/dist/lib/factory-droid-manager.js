...
export class FactoryDroidManager {
    ...
    /**
     * 加载通用 Provider 配置到 Factory Droid
     */
    loadProviderConfig(providerId, providerDef, userConfig) {
        // 如果是 GLM Coding Plan，委托给 loadGLMConfig
        if (providerId.startsWith('glm_coding_plan')) {
            return this.loadGLMConfig(providerId, userConfig.apiKey);
        }
        const currentConfig = this.getConfig();
        // 确定模型名：取 userConfig 中第一个模型名，否则用 providerDef.defaultModel
        const models = userConfig.models;
        const modelName = (Array.isArray(models) && models.length > 0)
            ? models[0]
            : (providerDef.defaultModel || 'default');
        // 构建新的 customModel
        const customModel = {
            displayName: providerDef.name + ' [' + providerId + ']',
            model: modelName,
        ...
        // 过滤掉旧的含 providerId 的 customModel（通过 displayName 判断）
        const existingModels = (currentConfig.customModels || []).filter(m => !m.displayName.includes(providerId));
        const newConfig = {
            ...currentConfig,
            customModels: [...existingModels, customModel]
        };
        this.saveConfig(newConfig);
    }
    /**
    ...
    unloadProviderConfig(providerId) {
        // 如果是 GLM Coding Plan，委托给 unloadGLMConfig
        if (providerId.startsWith('glm_coding_plan')) {
            return this.unloadGLMConfig();
        }
        const currentConfig = this.getConfig();
        // 过滤掉 displayName 含 providerId 的 customModel
        if (currentConfig.customModels) {
            currentConfig.customModels = currentConfig.customModels.filter(m => !m.displayName.includes(providerId));
            // 如果 customModels 为空，删除字段
            if (currentConfig.customModels.length === 0) {
                delete currentConfig.customModels;
            }
    ...
    detectCurrentConfig() {
        try {
            ...
            // Check for GLM Coding Plan
            const glmModel = config.customModels.find(m => m.displayName?.includes('GLM Coding Plan'));
            if (glmModel) {
                const baseUrl = glmModel.baseUrl;
                const planId = baseUrl?.includes('z.ai')
                    ? 'glm_coding_plan_global'
                    : baseUrl?.includes('bigmodel.cn')
                        ? 'glm_coding_plan_china'
                        : null;
                return { providerId: planId, apiKey: glmModel.apiKey || null, modelName: glmModel.model || null };
            }
            // Check for known provider IDs in displayName format: "ProviderName [providerId]"
            ...
            for (const model of config.customModels) {
                const match = model.displayName?.match(/\[([^\]]+)\]/);
                if (match && knownProviderIds.includes(match[1])) {
                    return {
                        providerId: match[1],
            ...
            // Fallback: return first non-GLM customModel
            const firstModel = config.customModels.find(m => !m.displayName?.includes('GLM Coding Plan'));
            if (firstModel) {
                const match = firstModel.displayName?.match(/\[([^\]]+)\]/);
                return {
                    providerId: match ? match[1] : null,
                    apiKey: firstModel.apiKey || null,
                    modelName: firstModel.model || null
                };
            }
            return { providerId: null, apiKey: null, modelName: null };
        }
